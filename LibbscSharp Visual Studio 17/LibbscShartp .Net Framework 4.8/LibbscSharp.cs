...
namespace LibbscSharp
{
    public class LibscSharp
    {
        ...
        {
            int result = Compressor.CompressOmp(inputStream, outputStream, 10 * 1024 * 1024, 0, 0, 0, 1, 2);
            return result;
        }
        ...
        {
            int result = Compressor.CompressOmp(inputStream, outputStream, 10 * 1024 * 1024, Numthreads, 0, 0, 1, 2);
            return result;
        }

        public int BSCCompress(Stream inputStream, Stream outputStream, int CompressionLevel, int Numthreads, int BlobkSize)
        {
            int result = Compressor.CompressOmp(inputStream, outputStream, BlobkSize, Numthreads, 0, 0, 1, 2);
            return result;
        }
